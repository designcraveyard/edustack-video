...
| Human-character video | `fal-ai/wan/v2.7/image-to-video` (720p default) when `brief.character_mode == "human"`; Seedance 1.5 Pro otherwise | Seedance + most i2v models distort/reject real human faces in keyframes. Wan 2.7 explicitly supports humans; smoke-validated 2026-05-18 with 4 parallel clips at 720p, identity locked across all four. Routing in [`scripts/phase_clips._resolve_video_stage`](scripts/phase_clips.py). |
| Clip-gen concurrency | ThreadPoolExecutor, default 4 workers | Each beat's gen + QA + retry loop is independent — safe to parallelize. Knob: `models.yaml:<stage>.concurrency` or `--concurrency` flag. |
...
| Book-from-keyframes | Standalone post-video path (`/create-book-from-keyframes`) that reuses the video's storyboard keyframes as book illustrations — birefnet/v2 for BG removal when requested, then Phase B3 (Pillow composer) to A4P/A3L 300 DPI. Skips Phase B1 (book-plan) and B2 (book-render) entirely. | Preserves visual continuity with the video. Cheaper than re-generating via gpt-image-2 (no fresh image gen cost, just one birefnet call per page where requested). Useful when the user wants a book identical in art to the video. Mutually exclusive with the default `brief.book.page_count_target > 0` flow on the same run (both write to `<run-dir>/book/`). |
